...
int main() {
    ...
    int Height = src.rows;
    int Width = src.cols;
    cout << "picture size:" << Height << "x" << Width << endl;
    ...
    for (int i = 0; i < 1000; i++) {
        RGB2YUVSSE_3(Src, Y, U, V, Width, Height, Stride);

        timeval start_time,end_time;
//        gettimeofday(&start_time, NULL);
//		YUV2RGBSSE_4(Y, U, V, Dest, Width, Height, Stride); //用了四个线程，而sws_scale只用一个
//		gettimeofday(&end_time, NULL);
//        float use_time = 1000 * (end_time.tv_sec - start_time.tv_sec) +
//                                   (end_time.tv_usec - start_time.tv_usec) / 1000.0;
//        cout << "use_time:" << use_time << "ms" << endl;   //640x480时sws_scale,yuv2bgr只需要1ms左右
        ...
        int out_size = av_image_get_buffer_size(AV_PIX_FMT_BGR24,Width,Height,1);
        uint8_t *BGR24_buffer  = (uint8_t *)malloc(out_size);
        av_image_fill_arrays(dst.data, dst.linesize, BGR24_buffer, AV_PIX_FMT_BGR24, Width, Height, 1);
        //avpicture_fill((AVPicture *)&dst, BGR24_buffer, AV_PIX_FMT_BGR24, out_width, out_height);//初始化dst



        int in_size = av_image_get_buffer_size(AV_PIX_FMT_YUV420P,Width,Height,1);
        uint8_t *yuv_buffer  = (uint8_t *)malloc(in_size);
        av_image_fill_arrays(avframe.data, avframe.linesize, yuv_buffer, AV_PIX_FMT_YUV420P, Width, Height, 1);
        avframe.data[0] = Y;
        avframe.data[1] = U;
        avframe.data[2] = V;
        avframe.width = Width;
        ...
        convert_ctx = sws_getContext(avframe.width, avframe.height, (enum AVPixelFormat)avframe.format, //input
                                     Width,  Height, AV_PIX_FMT_BGR24, //output
                                     SWS_FAST_BILINEAR, NULL, NULL, NULL);
        sws_scale(convert_ctx, avframe.data, avframe.linesize, 0, avframe.height,
                  dst.data, dst.linesize);
        sws_freeContext(convert_ctx);
        gettimeofday(&end_time, NULL);
        float use_time = 1000 * (end_time.tv_sec - start_time.tv_sec) +
                         (end_time.tv_usec - start_time.tv_usec) / 1000.0;
        cout << "use_time2:" << use_time << "ms" << endl;   //640x480时sws_scale,yuv2bgr只需要1ms左右
//        Mat dstMat(Height, Width, CV_8UC3, dst.data[0]);
//        imwrite("res1.jpg", dstMat);
        free(yuv_buffer);
        free(BGR24_buffer);
